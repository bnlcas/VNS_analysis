function VNS_dat = VNS_process_raw(root_dir, ekg_ch,electrodes, varargin)
...
% 
% 'load_env' - default false (load envelope data)
% 
...
%
% 'load_heart_rate' - default false (loads the heart rate in from the EKG)
%
%
%% Output:
...
if length(ekg_ch) > 2
    warning('More than 2 channels listed as ekg_ch')
end
...
if ~isempty(find(strcmpi(varargin,'load_env')))
    ...
    load_envelope = false;       % Load the bands for envelope
end
...
if ~isempty(find(strcmpi(varargin,'load_heart_rate')));
    load_heart_rate = varargin{find(strcmpi(varargin,'load_heart_rate'))+1};
else
    load_heart_rate = false;
end
...
%% Load anatomy file:
tmp = strsplit(root_dir,'/');
anatomy_file = [strjoin(tmp(1:(end-1)),'/') '/BrainPlot/clinical_elecs_all.mat'];
if exist(anatomy_file) == 2
    load([strjoin(tmp(1:(end-1)),'/') '/BrainPlot/clinical_elecs_all.mat']);
...
if load_heart_rate
    heart_rate = zeros(1,sum(dat_length_ds));
    tachogram_pad = 1000;
    tachogram_x = zeros(1,tachogram_pad*length(test_files));
    tachogram_y = zeros(1,tachogram_pad*length(test_files));
    tachogram_ind = 1;
    tachogram_time = 0;
end
...
for k = 1:length(test_files)
    ...
    tsvec = datevec((timestamps - (7*3600))/86400 + datenum(1970,1,1));
        %duration=(tsvec(end,5)*60+tsvec(end,6))-(tsvec(1,5)*60+tsvec(1,6));%total time
    duration = (timestamps(end)-timestamps(1));
        fp = [fp, round(length(timestamps)/duration)]; %samples/sec
    ...
    if load_vns_phase
        if length(ekg_ch) > 1
            vns_blk = data(ekg_ch(1),:) - data(ekg_ch(2),:);
        else
            vns_blk = data(ekg_ch,:);
        end
...
%         end
    ...
    if load_raw
        ...
        if length(ekg_ch) > 1
            vns_raw(:,file_onset_inds_full(k):file_offset_ind_full(k)) = (data(ekg_ch(1),:) - data(ekg_ch(2),:))';
        else
    ...
    if load_heart_rate
        if length(ekg_ch) > 1
            ekg_block = data(ekg_ch(1),:) - data(ekg_ch(2),:);
        else
            ekg_block = data(ekg_ch,:);
        end
        [~,qrs_i_raw,delay]=pan_tompkin(ekg_block,fs_in,false);
        %qrs_i_raw = qrs_i_raw - delay; % subtracts the phase shift
        %qrs_i_raw(qrs_i_raw<1) = [];
        rr_intervals = diff(qrs_i_raw)/fs_in; % the time (in seconds) between peaks
        rr_indecies = (qrs_i_raw(2:end) + qrs_i_raw(1:(end-1)))/2; % mid point of each interval
        % downsample indecies
        rr_indecies_ds = round(rr_indecies*fs_out/fs_in);
        % interpolate data
        first_ind = 1; last_ind = (1+file_offset_ind_ds(k)-file_onset_inds_ds(k));
        
        rr_interp = interp1(rr_indecies_ds, rr_intervals, first_ind:last_ind, 'linear', 'extrap');
        heart_rate(file_onset_inds_ds(k):file_offset_ind_ds(k)) = rr_interp;
        

        %% Add Tachogram:
        rr_time = rr_indecies/fs_in;
        
        tach_range = tachogram_ind:(tachogram_ind+length(rr_intervals)-1);
        tachogram_x(tach_range) = rr_time + tachogram_time;
        tachogram_y(tach_range) = rr_intervals;
        
        tachogram_time = tachogram_time + length(ekg_block)/fs_in;
        tachogram_ind = tachogram_ind + length(rr_intervals);
    end
...
if load_heart_rate
    VNS_dat.heart_rate = heart_rate;
    VNS_dat.tachogram_x = tachogram_x(1:(tachogram_ind-1));
    VNS_dat.tachogram_y = tachogram_y(1:(tachogram_ind-1));
end
