function [out_table, effect_sizes, pvals] = generate_block_analysis_lump_phase(VNS_dat, varargin)
...
%
% Inputs:
% VNS_dat - VNS data structure that includes phase data
%
% Variable Inputs:
...
for b = 1:length(classes)
    ...
    for k = 1:dims(1)
        pvals(k,b) = circ_kuiper_eq(ecog_post(k,:)',ecog_pre(k,:)');
    end
    data_table(b,1) = 100*sum(pvals(:,b)<p_thresh)/sum(is_good_chan);
    effect_sizes(:,b) = (circ_mean(ecog_post,[],2) - circ_mean(ecog_pre,[],2))./circ_std([ecog_post ecog_pre],[],[],2);
