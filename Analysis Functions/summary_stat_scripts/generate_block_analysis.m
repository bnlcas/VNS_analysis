...
% pariwise correlations between the time blocks when stimulation is present
...
% 
% Here significance is determined by the T-tes Test, except the pairwise
...
for b = 1:length(classes)
    ...
    for k = 1:size(erps,3)
        pw_corr_trial_pre(:,:,k) = corrcoef(squeeze(erps(:,is_pre_onset,k))','rows','pairwise');
        pw_corr_trial_post(:,:,k) = corrcoef(squeeze(erps(:,is_post_onset,k))','rows','pairwise');
    end
